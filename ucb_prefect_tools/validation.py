...
def generate_schema(df: pd.DataFrame, to_script=False, path: PathLike | None = None):
    ...
    if to_script:
        if not path:
            raise NotImplementedError("Schemas in script form must be written to a file, please specify a path")
        schema.to_script(path)
        return None
    return schema.to_yaml(path)

@task
def validate_dataframe(df: pd.DataFrame, schema: pa.DataFrameSchema = None, yaml_path: PathLike = None, warn=False):
    ...
    if not schema:
        if not yaml_path:
            raise ValueError("Must provide either a schema or a path to a YAML file containing a schema.")
        schema = from_yaml(yaml_path)
    try:
        schema.validate(df, lazy=True)
    except pa.errors.SchemaErrors as exc:
        if warn:
            get_run_logger().warning("WARNING - Dataframe failed validation:\n" + json.dumps(exc.message, indent=2))
        else:
            raise exc
